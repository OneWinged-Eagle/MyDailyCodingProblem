...
def makeTree(leveledNodes: List[Tuple[int, Node]]) -> Node:
	...
	for i, (l, _) in enumerate(leveledNodes[1:]):
		...
		if l == currLevel + 1:
			...
			if childNode.type == Type.file or (childNode.type == Type.dir and
			                                   childNode.children):
				longestChildLength = childNode.longestChildLength + childNode.length
				if longestChildLength > currNode.longestChildLength:
					currNode.longestChildLength = longestChildLength
...
def longestPath(fileSystem: str) -> int:
	...
	if fileSystemTree.longestChildLength > 0:
		return fileSystemTree.length + fileSystemTree.longestChildLength
	return 0
